...
import {
	...
	getDefaultCommandComposeProfilesOrServices,
...
async function compose(
	...
	message: string,
	dockerComposeFileUri?: vscode.Uri | string,
	...
	for (const command of commands) {
		...
		for (const item of selectedItems) {
			...
			if (!terminalCommand.args?.length) {
				...
				terminalCommand =
					await addDefaultCommandServicesOrProfilesIfNeeded(
						context,
						folder,
						terminalCommand,
						preselectedServices,
...
async function addDefaultCommandServicesOrProfilesIfNeeded(
	context: IActionContext,
	workspaceFolder: vscode.WorkspaceFolder,
	command: VoidCommandResponse,
	preselectedServices: string[],
	preselectedProfiles: string[],
): Promise<VoidCommandResponse> {
	const commandWithoutPlaceholders = {
		...command,
		args: command.args.filter((arg) =>
			typeof arg === "string"
				? !serviceListPlaceholder.test(arg) &&
					!profileListPlaceholder.test(arg)
				: !serviceListPlaceholder.test(arg.value) &&
					!profileListPlaceholder.test(arg.value),
		),
	};

	const { services, profiles } =
		await getDefaultCommandComposeProfilesOrServices(
			context,
			workspaceFolder,
