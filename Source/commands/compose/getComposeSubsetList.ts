...
export async function getDefaultCommandComposeProfilesOrServices(
	context: IActionContext,
	workspaceFolder: vscode.WorkspaceFolder,
	composeCommand: VoidCommandResponse,
	preselectedServices?: string[],
	preselectedProfiles?: string[],
): Promise<{ services: CommandLineArgs; profiles: CommandLineArgs }> {
	const profiles = await getDefaultCommandServiceSubsets(
		workspaceFolder,
		composeCommand,
		"profiles",
	);
	...
	if (preselectedServices?.length && preselectedProfiles?.length) {
		throw new Error(
			vscode.l10n.t(
				"Cannot specify both services and profiles to start. Please choose one or the other.",
			),
		);
	}
	...
	if (preselectedProfiles?.length) {
		...
		useProfiles =
			"profiles" ===
			(
				await context.ui.showQuickPick(profilesOrServices, {
					placeHolder: vscode.l10n.t(
						"Do you want to start services or profiles?",
					),
				})
			).data;
	}
	...
	return {
		profiles: useProfiles
			? await getDefaultCommandComposeProfileList(
					context,
					workspaceFolder,
					composeCommand,
					profiles,
			...
			: [],
		services: !useProfiles
			? await getDefaultCommandComposeServiceList(
					context,
					workspaceFolder,
					composeCommand,
					preselectedServices,
...
export async function getComposeProfilesOrServices(
	context: IActionContext,
	workspaceFolder: vscode.WorkspaceFolder,
	composeCommand: string,
	preselectedServices?: string[],
	preselectedProfiles?: string[],
): Promise<{ services: string | undefined; profiles: string | undefined }> {
	const profiles = await getServiceSubsets(
		workspaceFolder,
		composeCommand,
		"profiles",
	);
...
async function getDefaultCommandComposeProfileList(
	context: IActionContext,
	workspaceFolder: vscode.WorkspaceFolder,
	composeCommand: VoidCommandResponse,
	prefetchedProfiles?: string[],
	preselectedProfiles?: string[],
): Promise<CommandLineArgs> {
	const profiles =
		prefetchedProfiles ??
		(await getDefaultCommandServiceSubsets(
			workspaceFolder,
			composeCommand,
			"profiles",
		));
	...
	const result = preselectedProfiles?.length
		? preselectedProfiles
		: await pickSubsets(context, "profiles", profiles, previousChoices);
...
async function getDefaultCommandComposeServiceList(
	context: IActionContext,
	workspaceFolder: vscode.WorkspaceFolder,
	composeCommand: VoidCommandResponse,
	preselectedServices?: string[],
): Promise<CommandLineArgs> {
	const services = await getDefaultCommandServiceSubsets(
		workspaceFolder,
		composeCommand,
		"services",
	);
	...
	const result = preselectedServices?.length
		? preselectedServices
		: await pickSubsets(context, "services", services, previousChoices);
...
export async function getComposeProfileList(
	context: IActionContext,
	workspaceFolder: vscode.WorkspaceFolder,
	composeCommand: string,
	prefetchedProfiles?: string[],
	...
	const profiles =
		prefetchedProfiles ??
		(await getServiceSubsets(workspaceFolder, composeCommand, "profiles"));
...
export async function getComposeServiceList(
	context: IActionContext,
	workspaceFolder: vscode.WorkspaceFolder,
	composeCommand: string,
	preselectedServices?: string[],
): Promise<string> {
	const services = await getServiceSubsets(
		workspaceFolder,
		composeCommand,
		"services",
	);
...
async function pickSubsets(
	...
	const chosenSubsets = await context.ui.showQuickPick(pickChoices, {
		canPickMany: true,
		placeHolder: label,
		isPickSelected: (pick) =>
			previousChoices.some(
				(previous) =>
					(pick as IAzureQuickPickItem<string>).data === previous,
			),
	});
...
async function getDefaultCommandServiceSubsets(
	workspaceFolder: vscode.WorkspaceFolder,
	composeCommand: VoidCommandResponse,
	type: SubsetType,
	...
	try {
		return await runWithDefaults(() => configCommand, {
...
async function getServiceSubsets(
	workspaceFolder: vscode.WorkspaceFolder,
	composeCommand: string,
	type: SubsetType,
): Promise<string[] | undefined> {
	...
	try {
		...
		const configCommand = composeCommand.replace(
			composeCommandReplaceRegex,
			`config --${type}`,
		);

		const { stdout } = await execAsync(configCommand, {
			cwd: workspaceFolder.uri.fsPath,
		});
