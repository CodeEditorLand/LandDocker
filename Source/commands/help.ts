...
export async function help(context: IActionContext): Promise<void> {
	const items: HelpMenuItem[] = [
		{
			label: vscode.l10n.t("Get started with Docker..."),
			handler: getStarted,
			telemetryID: "getStarted",
		},
		{
			label: vscode.l10n.t("Review Docker extension issues..."),
			handler: reviewIssues,
			telemetryID: "reviewIssues",
		},
		{
			label: vscode.l10n.t("Report Docker extension issue..."),
			handler: reportIssueFromHelpMenu,
			telemetryID: "reportIssue",
		},
		{
			label: vscode.l10n.t("Edit settings..."),
			handler: editSettings,
			telemetryID: "editSettings",
		},
	];
...
async function reviewIssues(): Promise<void> {
	void vscode.env.openExternal(
		vscode.Uri.parse("https://github.com/microsoft/vscode-docker/issues"),
	);
}
