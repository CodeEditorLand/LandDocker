...
type TemplatePicker = (
	items: IAzureQuickPickItem<CommandTemplate>[],
	options: IAzureQuickPickOptions,
) => Promise<IAzureQuickPickItem<CommandTemplate>>;
...
export async function selectBuildCommand(
	...
	return await selectCommandTemplate(
		...
		folder,
		{
			...
			"containerCommand": await ext.runtimeManager.getCommand(),
		},
	);
}

export async function selectRunCommand(
	context: IActionContext,
	fullTag: string,
	imageId: string,
	interactive: boolean,
	...
	if (exposedPorts) {
		portsString = exposedPorts
			.map(
				(pb) =>
					`-p ${pb.containerPort}:${pb.containerPort}${pb.protocol ? "/" + pb.protocol : ""}`,
			)
			.join(" ");
	}
	...
	return await selectCommandTemplate(
		...
		{
			"tag": tagOrImageId,
			"exposedPorts": portsString,
			"containerCommand": await ext.runtimeManager.getCommand(),
		},
	);
}

export async function selectAttachCommand(
	context: IActionContext,
...
export async function selectLogsCommand(
	context: IActionContext,
	containerName: string,
	imageName: string,
	containerId: string,
...
export async function selectComposeCommand(
	context: IActionContext,
	folder: vscode.WorkspaceFolder,
	composeCommand: "up" | "down" | "upSubset",
...
export async function selectCommandTemplate(
	...
	templatePicker: TemplatePicker = (i, o) =>
		actionContext.ui.showQuickPick(i, o), // Default is the normal ext.ui.showQuickPick (this longer syntax is because doing `ext.ui.showQuickPick` alone doesn't result in the right `this` further down)
	getCommandSettings: () => CommandSettings = () =>
		vscode.workspace
			.getConfiguration("docker")
			.inspect<string | CommandTemplate[]>(`commands.${command}`),
): Promise<VoidCommandResponse> {
	...
	if (
		...
		(commandSettings.workspaceFolderValue || commandSettings.workspaceValue)
	) {
	...
	templateMatrix.push(
		getConstrainedTemplates(actionContext, userTemplates, matchContext),
	);
	...
	templateMatrix.push(
		getConstrainedTemplates(actionContext, defaultTemplates, matchContext),
	);
	...
	if (!selectedTemplate) {
		throw new Error(
			vscode.l10n.t(
				"No command template was found for command '{0}'",
				command,
			),
		);
	}

	const isDefault = defaultTemplates.some(
		(t) => t.template === selectedTemplate.template,
	);
	actionContext.telemetry.properties.isDefaultCommand = isDefault
		? "true"
		: "false";
	actionContext.telemetry.properties.isCommandRegexMatched =
		selectedTemplate.match ? "true" : "false";

	let resolvedCommand = resolveVariables(
	...
	const argsRegex = /(?:"[^"]*")|(?:[^"\s]*)/g;
	const commandAndArgs = resolvedCommand
		.match(argsRegex)
	...
	const resolvedArgs = commandAndArgs
		.slice(1)
		.map((arg) =>
			arg.startsWith('"') ? quoted(arg.replace(/"/g, "")) : arg,
		);
...
async function quickPickTemplate(
	...
	const items: IAzureQuickPickItem<CommandTemplate>[] = templates.map(
		(template) => {
			return {
...
function getConstrainedTemplates(
	actionContext: IActionContext,
	templates: CommandTemplate[],
	matchContext: string[],
): CommandTemplate[] {
	return templates.filter((template) => {
		...
		return isMatchConstraintSatisfied(
			actionContext,
			matchContext,
			template.match,
		);
...
function getUnconstrainedTemplates(
	templates: CommandTemplate[],
): CommandTemplate[] {
	return templates.filter((template) => {
...
function isMatchConstraintSatisfied(
	actionContext: IActionContext,
	matchContext: string[],
	...
	try {
		...
		// Don't wait
		void actionContext.ui.showWarningMessage(
			vscode.l10n.t(
				"Invalid match expression '{0}'. This template will be skipped.",
				match,
			),
		);
	}
...
function toCommandTemplateArray(
	maybeTemplateArray: CommandTemplate[] | string | undefined,
): CommandTemplate[] {
