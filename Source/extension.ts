...
function initializeExtensionVariables(ctx: vscode.ExtensionContext): void {
	...
	ext.outputChannel = new AzExtLogOutputChannelWrapper(
...
export async function activateInternal(
	...
	perfStats: { loadStartTime: number; loadEndTime: number | undefined },
		...
		async (activateContext: IActionContext) => {
			...
			ext.experimentationService = await createExperimentationService(
				...
				process.env.VSCODE_DOCKER_TEAM === "1"
					? tas.TargetPopulation.Team
					: undefined, // If VSCODE_DOCKER_TEAM isn't set, let @microsoft/vscode-azext-utils decide target population
			);
...
function setEnvironmentVariableContributions(): void {
	const settingValue: NodeJS.ProcessEnv = vscode.workspace
	...
	for (const key of Object.keys(settingValue)) {
		ext.context.environmentVariableCollection.replace(
			key,
			settingValue[key],
		);
	}
		...
		for (const item of params.items) {
			...
			if (item.scopeUri) {
				config = vscode.workspace.getConfiguration(
					item.section,
					dockerfileLanguageClient.protocol2CodeConverter.asUri(
						item.scopeUri,
					),
				);
			} else {
		...
		ctx.subscriptions.push(
			vscode.workspace.onDidChangeConfiguration(
				async (e: vscode.ConfigurationChangeEvent) => {
					...
					void dockerfileLanguageClient.sendNotification(
...
function activateComposeLanguageClient(ctx: vscode.ExtensionContext): void {
	// Don't wait
	void callWithTelemetryAndErrorHandling(
		"docker.composelanguageclient.activate",
		async (context: IActionContext) => {
			context.telemetry.properties.isActivationEvent = "true";
			...
			registerEvent(
				"compose-langserver-event",
				composeLanguageClient.onTelemetry,
