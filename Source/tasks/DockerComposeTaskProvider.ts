...
import {
	...
	DockerComposeTaskDefinitionBase,
} from "./DockerComposeTaskDefinitionBase";
import { DockerTaskProvider } from "./DockerTaskProvider";
import {
	DockerComposeTaskContext,
...
export class DockerComposeTaskProvider extends DockerTaskProvider {
	public constructor() {
		super("docker-compose", undefined);
	}

	protected async executeTaskInternal(
		context: DockerComposeTaskContext,
	...
	private async validateResolvedDefinition(
		...
		if (dockerCompose.up && dockerCompose.down) {
			throw new Error(
				l10n.t(
					'Both "up" and "down" properties are present in the docker-compose task.',
				),
			);
		}
		...
		if (!dockerCompose.up && !dockerCompose.down) {
			throw new Error(
				l10n.t(
					'Neither "up" nor "down" properties are present in the docker-compose task.',
				),
			);
		}
		...
		if (dockerCompose.up?.services && dockerCompose.up?.profiles) {
			throw new Error(
				l10n.t(
					'Both "services" and "profiles" are present in the docker-compose task\'s "up" property.',
				),
			);
		}
			...
			if (
				!(await fse.pathExists(
					path.resolve(
						...
						resolveVariables(file, context.folder),
					),
				))
			) {
				throw new Error(
					l10n.t(
						"One or more docker-compose files does not exist or could not be accessed.",
					),
				);
			}
		}
		...
		if (
			...
			!(await fse.pathExists(
				path.resolve(
					context.folder.uri.fsPath,
					resolveVariables(dockerCompose.envFile, context.folder),
				),
			))
		) {
			throw new Error(
				l10n.t(
					"Environment file does not exist or could not be accessed.",
				),
			);
		}
	}
